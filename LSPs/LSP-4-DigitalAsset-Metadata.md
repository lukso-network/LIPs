...
|  `1`  | `NFT`  | If the contract is LSP7 or LSP8, then the `LSP4Metadata` represents the information of a **single** NFT, that has multiple ownable amounts or IDs. If its an LSP7 contract, the `decimals` function MUST return `0`. Each individual token ID COULD have its own custom metadata specific for the token ID, but MUST NOT be different NFTs. Those COULD be set using `LSP8TokenIdType` and `LSP8MetadataTokenURI`. [See LSP8 for details](./LSP-8-IdentifiableDigitalAsset.md).  |
