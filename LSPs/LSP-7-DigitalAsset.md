...
This standard defines an interface for tokens where minting and transfering is specified with an amount of tokens. It is based on [ERC20](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md) with some ideas from [ERC777](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md).
...
- `force` when set to TRUE, `to` may be any address; when set to False `to` must be a contract that supports [LSP1-UniversalReceiver](https://github.com/lukso-network/LIPs/blob/master/LSPs/LSP-1-UniversalReceiver.md) and not revert.
...
A implementation can be found in the [lukso-network/lsp-smart-contracts](https://github.com/fanzone-media/lsp-smart-contracts/blob/permissions/contracts/LSP7/LSP7Core.sol);
